...
bone1=['C:\Users\ajoshi\Google Drive\MRI and Ground Truth Images/Volunteer1_GroundTruth.img'];
bone2=['C:\Users\ajoshi\Google Drive\MRI and Ground Truth Images/Volunteer2_GroundTruth.img'];
bone2vol=['C:\Users\ajoshi\Google Drive\MRI and Ground Truth Images/Volunteer2_VIBE_we.img'];
bone1vol=['C:\Users\ajoshi\Google Drive\MRI and Ground Truth Images/Volunteer1_VIBE_we.img'];
...
b2vol=load_nii(bone2vol);
b2vol=resample_avw(b2vol,[128,128,128]);
b1vol=load_nii(bone1vol);
...
for lab=labs'
    ...
    b1=readdfs([bone1(1:end-3),sprintf('bone%d10000.dfs',lab)]);
    b2=readdfs([bone2(1:end-3),sprintf('bone%d10000.dfs',lab)]);

    tic;    
        w.vertices = register_pointset_icp(b1.vertices,b2.vertices,b2.faces);
    toc
%  tic;  [k,d]=dsearchn(w.vertices,bone1.vertices);toc
%   d2=smooth_surf_function(bone1,d,.8,.8);
...
save_nii(bw,[bone1(1:end-4),'_warped','.nii.gz']);
view_nii(bw);
view_nii(b2v)
view_nii(b1v)
save /home/ajoshi/Desktop/tmp2
...
b2volo=load_nii(bone2vol);b2labo=load_nii(bone2);
b1volo=load_nii(bone1vol);b1labo=load_nii(bone1);
...
save_nii(bwlab,[bone1(1:end-4),'_warped','.nii.gz']);
